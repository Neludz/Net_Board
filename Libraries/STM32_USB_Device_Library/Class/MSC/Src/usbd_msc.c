...
static uint32_t mem_2[(sizeof(USBD_MSC_BOT_HandleTypeDef)/4)+1];
...
uint8_t USBD_MSC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  pdev->pClassData_2 = mem_2;// USBD_malloc(sizeof(USBD_MSC_BOT_HandleTypeDef));

  if (pdev->pClassData_2 == NULL)
  {
    return USBD_FAIL;
  }
                         ...
                         uint8_t cfgidx)
{
  ...
  if (pdev->pClassData_2 != NULL)
  {
    USBD_free(pdev->pClassData_2);
    pdev->pClassData_2  = NULL;
  }
    ...
    case USB_REQ_TYPE_CLASS:
      switch (req->bRequest)
      {
        case BOT_GET_MAX_LUN:
          ...
          {
            hmsc->max_lun = (uint32_t)((USBD_StorageTypeDef *)pdev->pUserData_2)->GetMaxLun();
